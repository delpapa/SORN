...
while not os.path.exists(c.logfilepath): 
    time.sleep(0.1) # not rank 0 and not created yet

# Create one logger for all simulations
dlog.progress("NUMBER_OF_SIMS = %d"%c.cluster.NUMBER_OF_SIMS)
result_path = utils.logfilename('result.h5')
...
for num_local in range(c.cluster.NUMBER_LOCAL):
    ...
    if c.cluster.__contains__('params'):
        current_param = c.cluster.params[MPI.COMM_WORLD.rank]
    else:
        ...
        current_param = ((MPI.COMM_WORLD.rank*c.cluster.NUMBER_LOCAL)
